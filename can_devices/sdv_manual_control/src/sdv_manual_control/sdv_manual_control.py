...
"""
Welcome to sdv ROS manual control.
...
class ManualControl(CompatibleNode):
    ...
    def __init__(self, resolution):
        ...
        self.collision_subscriber = self.new_subscription(
            CollisionEvent, "/sdv/{}/collision".format(self.role_name),
    ...
    def on_lane_invasion(self, data):
        ...
        for marking in data.crossed_lane_markings:
            if marking is LaneInvasionEvent.LANE_MARKING_OTHER:
                text.append("Other")
            elif marking is LaneInvasionEvent.LANE_MARKING_BROKEN:
                text.append("Broken")
            elif marking is LaneInvasionEvent.LANE_MARKING_SOLID:
                text.append("Solid")
            else:
                text.append("Unknown ")
...
class KeyboardControl(object):
    ...
    def __init__(self, role_name, hud, node):
        ...
        self.vehicle_control_manual_override_publisher = self.node.new_publisher(
            Bool,
            "/sdv/{}/vehicle_control_manual_override".format(self.role_name),
        ...
        self.auto_pilot_enable_publisher = self.node.new_publisher(
            Bool,
            "/sdv/{}/enable_autopilot".format(self.role_name),
            qos_profile=fast_qos)

        self.vehicle_control_publisher = self.node.new_publisher(
            VehicleControl,
            "/sdv/{}/vehicle_control_cmd_manual".format(self.role_name),
            qos_profile=fast_qos)

        self.sdv_status_subscriber = self.node.new_subscription(
            Status,
            "/sdv/status",
            self._on_new_sdv_frame,
            qos_profile=10)
    ...
    def _on_new_sdv_frame(self, data):
        """
        callback on new frame

        As sdv only processes one vehicle control command per tick,
...
class HUD(object):
    ...
    def __init__(self, role_name, width, height, node):
        ...
        self.vehicle_info_subscriber = node.new_subscription(
            VehicleInfo,
            "/sdv/{}/vehicle_info".format(self.role_name),
        ...
        self.gnss_subscriber = node.new_subscription(
            NavSatFix,
            "/sdv/{}/gnss".format(self.role_name),
            self.gnss_updated,
            qos_profile=10)

        self.odometry_subscriber = node.new_subscription(
            Odometry,
            "/sdv/{}/odometry".format(self.role_name),
        ...
        self.sdv_status = Status()
        self.status_subscriber = node.new_subscription(
            Status,
            "/sdv/status",
            self.sdv_status_updated,
            qos_profile=10)
    ...
    def sdv_status_updated(self, data):
        """
        Callback on sdv status
        """
        self.sdv_status = data
    ...
    def update_info_text(self):
        ...
        if self.sdv_status.fixed_delta_seconds:
            fps = 1 / self.sdv_status.fixed_delta_seconds
        self._info_text = [
            'Frame: % 22s' % self.sdv_status.frame,
...
def main(args=None):
    ...
    resolution = {"width": 800, "height": 423}
