...
"""
✅ Replaced CosineAnnealingLR with CyclicLR for dynamic learning rate adaptation.
✅ Increased weight_decay to 5e-3 for better regularization.
✅ Added dropout (0.2) to the GRU layer to prevent overfitting.
✅ Manually reduced the learning rate at epoch 40 to fine-tune the model.
"""
...
scheduler = torch.optim.lr_scheduler.CyclicLR(optimizer, base_lr=5e-5, max_lr=5e-4, step_size_up=10, mode="triangular2")
...
with gpytorch.settings.memory_efficient(True):
    ...
    if epoch == 40:
        for param_group in optimizer.param_groups:
            param_group['lr'] *= 0.1  # Reduce LR manually at epoch 40
            # scheduler_plateau.step(avg_loss)
            # train_losses.append(avg_loss)
            # scheduler_plateau.step(avg_loss)
            print(f"Epoch {epoch+1}, Average Loss: {avg_loss:.10f}")
...
plt.savefig("/docker-ros/ws/src/tests/GRU/test3/training_loss.png")
# plt.show()
...
with open("/docker-ros/ws/src/tests/GRU/test3/test_results.txt", "w") as f:
...
plt.legend()
plt.grid(True)
plt.savefig("/docker-ros/ws/src/tests/GRU/test3/residual_predictions.png")
# plt.show()

# === 7. Save Model ===
print("Saving model...")
torch.save(model.state_dict(), "/docker-ros/ws/src/tests/GRU/test3/gru_residual_dynamics.pth")
...
model_script_path = "/docker-ros/ws/src/tests/GRU/test3/gru_residual_dynamics.pt"
